...
from .models import (
    ...
    Classroom, GradingScale, GradeRule, BuildingAccessLog
)
...
class UserAdmin(BaseUserAdmin):
    ...
    list_display = ('email', 'full_name', 'role', 'group', 'phone', 'is_active', 'is_staff')
...
class StudyGroupAdmin(admin.ModelAdmin):
    list_display = ('name', 'specialty', 'course', 'year_of_entry', 'graduation_year', 'is_active', 'created_at')
    ...
    fieldsets = (
        ('Основна інформація', {
            'fields': ('name', 'specialty', 'course')
        }),
        ('Навчальний період', {
            'fields': ('year_of_entry', 'graduation_year')
        }),
        ('Системна інформація', {
...
class SubjectAdmin(admin.ModelAdmin):
    ...
    fieldsets = (
        ...
        ('Навчальне навантаження', {
            'fields': ('credits', 'semester', 'hours_total', 'hours_lectures', 'hours_practicals')
        }),
        ('Системна інформація', {
            'fields': ('is_active', 'created_at', 'updated_at'),
            'classes': ('collapse',)
...
class TeachingAssignmentAdmin(admin.ModelAdmin):
    list_display = ('subject', 'teacher', 'group', 'academic_year', 'semester', 'is_active')
...
class EvaluationTypeAdmin(admin.ModelAdmin):
    ...
    fieldsets = (
        ('Основна інформація', {
            'fields': ('assignment', 'name', 'weight_percent')
        }),
...
class ScheduleTemplateAdmin(admin.ModelAdmin):
    list_display = ('get_group', 'get_day', 'lesson_number', 'get_subject', 'get_teacher', 'start_time', 'classroom', 'is_active')
    list_filter = ('is_active', 'group', 'day_of_week', 'subject', 'teacher', 'week_type')
...
class LessonAdmin(admin.ModelAdmin):
    list_display = ('date', 'start_time', 'get_subject', 'get_group', 'get_teacher', 'topic', 'is_cancelled')
    list_filter = ('is_cancelled', 'date', 'group', 'subject', 'teacher', 'evaluation_type')
    search_fields = ('topic', 'notes', 'homework', 'group__name', 'subject__name', 'teacher__full_name')
    ...
    fieldsets = (
        ...
        ('Розклад', {
            'fields': ('date', 'start_time', 'end_time', 'classroom')
        }),
        ('Зміст заняття', {
            'fields': ('topic', 'max_points', 'homework', 'materials')
        }),
        ('Статус', {
...
class StudentPerformanceAdmin(admin.ModelAdmin):
    list_display = ('get_student', 'get_lesson', 'earned_points', 'absence', 'is_bonus', 'graded_at')
    list_filter = ('is_bonus', 'absence', 'graded_at', 'lesson__date', 'lesson__subject', 'student__group')
    ...
    fieldsets = (
        ...
        ('Оцінювання', {
            'fields': ('earned_points', 'absence', 'is_bonus')
        }),
        ('Коментар', {
            'fields': ('comment',)
        }),
        ('Системна інформація', {
            'fields': ('graded_by', 'graded_at', 'version', 'created_at', 'updated_at'),
            'classes': ('collapse',)
        }),
    )
...
class AbsenceReasonAdmin(admin.ModelAdmin):
    list_display = ('code', 'description', 'is_respectful', 'order', 'color', 'is_active')
    ...
    fieldsets = (
        ('Основна інформація', {
            'fields': ('code', 'description', 'is_respectful')
        }),
        ('Відображення', {
            'fields': ('order', 'color')
        }),
...
class BuildingAccessLogAdmin(admin.ModelAdmin):
    list_display = ('get_student', 'timestamp', 'action', 'location', 'device_id', 'is_valid')
    ...
    fieldsets = (
        ('Основна інформація', {
            'fields': ('student', 'timestamp', 'action')
        }),
        ('Деталі доступу', {
            'fields': ('location', 'device_id', 'is_valid')
        }),
        ('Примітки', {
            'fields': ('notes',),
            'classes': ('collapse',)
